...
/**
 * EdierBot - a robot by (Edier Bernal)
 */
public class EdierBot extends AdvancedRobot 
{
	boolean movingForward;
	final double FIREPOWER = 3; // Max. power => violent as this robot can afford it!
	final double HALF_ROBOT_SIZE = 10; // Robot size is 36x36 units, so the half size is 18 units
	...
	private void handleGun() {
		...
		fireGunWhenReady();
         ...
         /**
	 * Metodo para moverse en caso de que quede pegado a la pared
	 */
        private void retirarPared()
        { 
            setTurnRight(45);	
	...
	private void updateEnemyTargetPositions() {
		...
		for (RobotData enemy : enemyMap.values()) {
			...
			if (discrim >= 0) {
				...
				// negative
                                //double t = (Math.min(t1,t2) + Math.max(t1,t2))/2;
